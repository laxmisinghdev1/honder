...
import { benefitsPhrasesByLanguage, catalogPhrasesByLanguage, contactDetails, floatingActions, getBenefits, getCategories, getCategoryFilters, getFaqs, getHeroStats, getProducts, getRolloutModules, heroPhrasesByLanguage, heroVideoUrl, navItems, shippingPhrasesByLanguage, tickerItemsByLanguage, type CategoryKey, type Product } from './data/hondar'
...
const heroVideoPoster = 'https://images.unsplash.com/photo-1517649763962-0c623066013b?auto=format&fit=crop&w=1600&q=80'
...
function App() {
  ...
  const footerNavLinks = useMemo(
    () => navItems.map((item) => ({ href: item.href, label: t(item.key) })),
    [t],
  )
  ...
  useEffect(() => {
    ...
    const minimumDelay = window.setTimeout(() => {
      if (document.readyState === 'complete' && !isDisposed) {
        setIsBooting(false)
      }
    }, 1050)
  ...
  return (
    <div className="site-shell">
      <div className={`loading-screen ${isBooting ? 'loading-screen--visible' : 'loading-screen--hidden'}`} aria-hidden={!isBooting}>
            ...
            <div className="product-grid">
              {filteredProducts.map((product, index) => (
                <AnimatedSection
                  ...
                  blur={8}
                >
                  <button className="product-card__visual" type="button" onClick={() => setSelectedProduct(product)}>
